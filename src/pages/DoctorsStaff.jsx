...
const StatusBadge = ({ status }) => {
  const base = "px-2 py-1 rounded-md text-xs font-medium flex items-center justify-center";
  return status === "Active" ? (
    <span className={`${base} bg-[#E1F4EE] text-[#56B597] text-[14px]`}>Active</span>
  ) : (
    <span className={`${base} bg-gray-200 text-gray-600 text-[14px]`}>Inactive</span>
  );
};
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-6">
        <div>
          <h1 className="text-[#000] font-medium text-[20px] md:text-2xl">Staff Management</h1>
        ...
        <button
          ...
          className="bg-[#0F21E4] text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-sm md:text-base"
        >
          + Add Staff
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3 mb-4">
        ...
        <div className="flex flex-wrap gap-3">
          <button className="flex items-center border border-[#A49F9F] px-3 py-2 rounded-lg text-[#838383] hover:bg-gray-100 text-[14px] md:text-sm">
      ...
      <div className="overflow-x-auto bg-white rounded-lg shadow border border-[#A49F9F]">
        <table className="w-full min-w-[600px] md:min-w-[800px] text-sm md:text-base">
          <thead className="bg-gray-50 text-left">
            <tr>
              <th className="p-3 text-[#717074] font-normal">#</th>
              <th className="p-3 text-[#717074] font-normal">Doctor ID</th>
              <th className="p-3 text-[#717074] font-normal">Name</th>
              <th className="p-3 text-[#717074] font-normal">Department</th>
              <th className="p-3 text-[#717074] font-normal">Status</th>
              <th className="p-3 text-[#717074] font-normal">Phone</th>
              <th className="p-3 text-[#717074] font-normal">Action</th>
            </tr>
          </thead>
          <tbody>
            {staffData.map((staff) => (
              <tr key={staff.id} className="border-b border-[#A49F9F] hover:bg-gray-50 transition-colors">
                <td className="p-3">
                  <input type="checkbox" />
                </td>
                <td className="p-3 text-[#717074]">{staff.id}</td>
                <td className="p-3 flex items-center gap-2 text-[#838383]">
      ...
      {/* Pagination */}
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mt-4 text-sm md:text-base text-[#2C2C2C]">
